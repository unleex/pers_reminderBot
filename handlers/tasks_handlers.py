...
async def add_task_command(msg: Message,state: FSMContext):
    ...
    if dueday != None:
        day_out = f'Записано на {dueday}, {duetime[0]}:{duetime[1]}'
...
async def confirm_add_task(clb: CallbackQuery,state:FSMContext,arqredis: ArqRedis):
    ...
    if new_homework.dueday:
        ...
        #add 2000 to year, since it decreases to tens(2024->24) somehow
        due_datetime = datetime(due_date[0]+2000, due_date[1], due_date[2], due_time[0], due_time[1]) 
        logger.info(msg=(f'New task added.\n\t'
